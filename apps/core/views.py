...
from .audit import get_client_ip, log_action, log_bulk_action
...
class HomepageView(View):
    """Public-facing homepage — no login required."""
...
def toggle_status(request, model_key, pk):
    ...
    log_action(
        ...
        AuditLog.TOGGLE,
        obj,
        changes={field: {"before": current, "after": not current}},
    )
...
def bulk_action(request, model_key):
    ...
    if action == "toggle":
        ...
        log_bulk_action(
            request, AuditLog.BULK_TOGGLE, config.model.__name__, count, selected_ids
        )
        invalidate_dashboard_cache()
