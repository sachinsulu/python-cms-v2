...
(function () {
  ...
  function ensureModal() {
    ...
    overlay
      .querySelector(".media-modal-cancel-btn")
      .addEventListener("click", closeModal);
    ...
    fileInput.addEventListener("change", async () => {
      ...
      try {
        const res = await fetch("/apanel/media-library/upload/", {
          method: "POST",
          body: fd,
        });
    ...
    // Event: select button
    overlay
      .querySelector(".media-modal-select-btn")
  ...
  async function loadLibrary() {
    ...
    try {
      const res = await fetch("/apanel/media-library/library/?type=image");
      const data = await res.json();
      ...
      body.innerHTML =
        '<p style="color:#ef4444; text-align:center;">Failed to load library.</p>';
    }
  }
    ...
    try {
      ...
      if (data.error) {
        if (typeof toast !== "undefined")
          toast.show(`Upload error: ${data.error}`);
        return;
      }
